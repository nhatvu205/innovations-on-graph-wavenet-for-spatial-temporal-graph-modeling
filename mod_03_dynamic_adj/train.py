...
try:
    ...
    from mod_03_dynamic_adj.engine import trainer


parser = argparse.ArgumentParser(description='Train Graph WaveNet with dynamic adjacency variant')
...
parser.add_argument('--save', type=str, default='./garage/gwnet_dynamic')
...
parser.add_argument('--model_variant', type=str, default='dynamic_adj', choices=['baseline', 'dynamic_adj'])
...
def main():
    ...
    dynamic_adj = args.model_variant == 'dynamic_adj'
    if dynamic_adj and not args.addaptadj:
        raise ValueError('dynamic_adj variant requires --addaptadj.')
    ...
    opt_params = sum(p.numel() for pg in engine.optimizer.param_groups for p in pg['params'])
    print(args)
    print(f'Resolved variant={args.model_variant} | dynamic_adj={dynamic_adj}', flush=True)
    print(f'Total params: {total_params:,}')
    print(f'Optimizer params: {opt_params:,}')
    if dynamic_adj and getattr(engine.model, 'dyn_adj', None) is not None:
        dyn_params = sum(p.numel() for p in engine.model.dyn_adj.parameters())
        print(f'dyn_adj params: {dyn_params:,}')
    assert total_params == opt_params, 'Optimizer thiếu params!'
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for i, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            if i % args.print_every == 0:
                print(
                    f'Iter: {i:03d}, Train Loss: {train_loss[-1]:.4f}, '
        ...
        epoch_ckpt_path = f'{args.save}_epoch_{epoch}_{round(mvalid_loss, 2):.2f}.pth'
        torch.save(engine.model.state_dict(), epoch_ckpt_path)
    ...
    print(
        f'\nOn average over 12 horizons, Test MAE: {np.mean(amae):.4f}, '
        f'Test MAPE: {np.mean(amape):.4f}, Test RMSE: {np.mean(armse):.4f}'
    )
    torch.save(engine.model.state_dict(), f'{args.save}_exp{args.expid}_best_epoch{best_epoch}_{best_val_loss:.2f}.pth')
